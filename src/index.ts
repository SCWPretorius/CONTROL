...
async function handleEvent(event: NormalizedEvent): Promise<void> {
  ...
  try {
    ...
    if (!decision) {
      ...
      // Queue the message for later retry if LLM was unavailable
    ...
    // Handle "Clarify" responses - don't execute as a skill, just log as a decision
    ...
    if (!result.success) {
      ...
      // Auto-send results for query/search skills that produce user-facing data
    ...
    // Queue message for retry on any unexpected error
    try {
      const queue = getMessageQueue();
      const messageId = await queue.enqueue('incoming', event.source, event.payload, event);
...
async function main(): Promise<void> {
  ...
  if (queueEnabled) {
    try {
      ...
      // Inject the event handler into the queue worker
      setEventHandler(handleEvent);
      
      // Inject message sender into queue worker
  ...
  // Load Home Assistant entities into context (runs in background)
  ...
  // Initial load with delay to ensure integration is fully initialized
